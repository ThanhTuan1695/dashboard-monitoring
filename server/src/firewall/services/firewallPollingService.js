...
async function loadCredential(device) {
  ...
  try {
    ...
    console.error('[firewall] failed to decrypt credential for device', device._id.toString(), err.message);
    return null;
  }
}
 ...
 * unchanged (see the note on the OFFLINE mapping below).
 */
async function poll(device, { forceDiscovery = false } = {}) {
  const discoveryResult = await ensureDiscovery(device, { force: forceDiscovery });
  const credential = await loadCredential(device);
  // An explicit vendor pick in the device form skips relying on confidence-scored
  // fingerprinting entirely — the user already told us what this device is.
  ...
  // are still reachable, managed devices; that nuance lives in the Firewall
