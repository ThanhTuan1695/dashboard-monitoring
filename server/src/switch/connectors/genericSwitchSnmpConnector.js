...
const PETH_MAIN_PSE_COLUMNS = { power: 3, consumptionPower: 4 };
 ...
 * hostname/uptime/interfaces/PoE-budget from here, regardless of vendor. No
 * CPU/memory/stack/LACP/STP/LLDP — those need vendor-specific MIBs this pass
 * doesn't implement, so those fields stay `null`/`[]` rather than guessed.
 * Every call is wrapped so a single unsupported OID degrades to `null`/`[]`
 * instead of failing the whole poll — this is unverified against real switch
 * hardware (no unit available in this environment), same caveat as the
 * firewall connector's FortiGate endpoints.
 */
function createGenericSwitchSnmpConnector({ host, timeoutMs = 3000, ...credentials } = {}) {
  ...
  return {
    ...
    async getNeighbors() {
      return []; // LLDP-MIB topology discovery is staged later (spec's own Stage 13) — capability recorded, no data source yet
    },
